            ...
            def resilient_send_notification_to_user(user, payload, ttl=0):
                ...
                for push_info in push_infos:
                    try:
                        ...
                        logger.warning(
                            f"WebPush failed for user {user.id} subscription {push_info.subscription.id}: {e}"
                        )
                        ...
                        logger.warning(
                            f"WebPush unexpected error for user {user.id} subscription {push_info.subscription.id}: {e}"
                        )
            ...
            def resilient_send_notification_to_group(group_name, payload, ttl=0, exclude_user_id=None):
                ...
                for push_info in push_infos:
                    try:
                        ...
                        status_code = e.response.status_code if e.response is not None else None
                        logger.warning(
                            f"WebPush group notify failed for subscription {push_info.subscription.id}: {e}"
                        )
                        if status_code in [403, 404, 410]:
                            try:
                                push_info.subscription.delete()
                        ...
                        logger.warning(
                            f"WebPush unexpected error for subscription {push_info.subscription.id}: {e}"
                        )
