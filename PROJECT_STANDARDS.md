...
- **Loading / skeletons:** Full-page navigations pick a destination overlay in `static/js/loading.js` (`classifyNavSkeleton`). Variants: `login`, `password-change`, `ticket-list`, `ticket-detail`, `dashboard`, `settings`, `kb-list`, `kb-detail`, `form` (create/edit pages), `news-list`. Unknown routes use the form-shaped `default`. HTMX modals use `#modal-skeleton` (form fields, not avatar rows). Settings tab HTMX uses `skeleton-table`. Password-change POST previews `login` (the view logs the user out). Do not try to predict force-password-change on login submit.
