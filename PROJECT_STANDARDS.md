...
- **Version:** the root `VERSION` file is shown as small muted text under sidebar Theme / Language (`core.context_processors.app_version`).
- **Loading / skeletons:** Full-page navigations pick a destination overlay in `static/js/loading.js` (`classifyNavSkeleton`). Variants: `login`, `password-change`, `ticket-list`, `ticket-detail`, `dashboard`, `settings`, `kb-list`, `kb-detail`, `form` (create/edit pages), `news-list`. Unknown routes use the form-shaped `default`. HTMX modals use `#modal-skeleton` (form fields, not avatar rows). Settings tab HTMX uses `skeleton-table`. Login submit disables the button and waits; destination skeleton is shown only after `HX-Redirect` (tickets, `?next=`, or password-change). Failed login stays on the form. Password-change POST previews `login` (the view logs the user out).
